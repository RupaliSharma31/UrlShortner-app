...
import java.util.Map; // <-- Map use karne ke liye required hai
        ...
        if (urlMapping != null) {
            return clickEventRepository.findByUrlMappingAndClickDateBetween(urlMapping, start, end).stream()
                    .collect(Collectors.groupingBy(click -> {
                        // Agar clickDate LocalDateTime hai toh .toLocalDate() use hoga, varna direct click.getClickDate()
                        Object date = click.getClickDate();
                        return date instanceof LocalDateTime ? ((LocalDateTime) date).toLocalDate() : (LocalDate) date;
                    }, Collectors.counting()))
                    .entrySet().stream()
                    .map(entry -> {
